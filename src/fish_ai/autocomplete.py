...
def get_instructions(
    ...
    if after_cursor.strip() == "":
        ...
        instructions[-1]["content"] += textwrap.dedent("""\
            ...
            with "{after_cursor}".
            """).format(before_cursor=before_cursor, after_cursor=after_cursor)
    ...
    if get_config("preview_pipe") == "True" and pipe != "":
        ...
        instructions[-1]["content"] = (
            ...
            """).format(command=pipe, output=short_output)
            + instructions[-1]["content"]
        )
        return instructions
    ...
    if filename:
        instructions[-1]["content"] = (
            ...
            """).format(filename=filename, file_contents=file_contents)
            + instructions[-1]["content"]
        )
...
def get_reload_command(commandline, cursor_position):
    return (
        "reload({install_dir}/bin/refine {commandline} "
        "{cursor_position} {completions_count} "
        "{instructions})+clear-query"
    ).format(
        ...
        cursor_position=cursor_position,
        completions_count=engine.get_config("refined_completions") or "3",
        instructions="{q}",
    )
...
def autocomplete():
    ...
    try:
        engine.get_logger().debug(
            "Autocompleting commandline: {}".format(
                before_cursor + "█" + after_cursor
            )
        )
        completions_count = int(engine.get_config("completions") or "5")
        engine.get_logger().debug(
            "Creating {} completions".format(completions_count)
        )
...
def refine_completions():
    ...
    engine.get_logger().debug(
        (
            "Refining {completions_count} completions "
    ...
    refined_completions = yield_completions(
        commandline, cursor_position, refined_completions_count, instructions
    )
